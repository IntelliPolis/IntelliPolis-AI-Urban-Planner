...
초기에는 AI가 추천한 시설이 산지나 실제 건설이 어려운 위치에 배치되는 문제가 있었습니다.

단순히 좌표만 기준으로 후보지를 선택하다 보니 지형이나 실제 활용 가능성을 충분히 고려하지 못했고, 현실성이 떨어지는 결과가 발생했습니다.

해결 방법

시설 추천 로직을 개선하여 후보지 데이터를 기반으로 실제 설치 가능성이 높은 위치만 선택하도록 수정했습니다.

또한 후보지의 면적, 생활권 인구, 활동량 등의 조건을 함께 고려하도록 로직을 보완하여 현실적인 시설 추천이 가능하도록 개선했습니다.

문제 2. 실시간 도로 혼잡도 데이터의 신뢰성

초기에는 실시간 도로 교통 혼잡 데이터를 이용하여 도로 개선안을 추천하는 기능을 구현했습니다.

하지만 실시간 교통 정보는 시간대와 일시적인 상황(사고, 공사, 행사 등)에 크게 영향을 받아 데이터가 지속적으로 변했습니다.

이로 인해 같은 지역이라도 분석 시점에 따라 서로 다른 결과가 나오는 문제가 있었고, 도시계획처럼 장기적인 의사결정을 지원하는 서비스에는 적합하지 않다고 판단했습니다.

해결 방법

실시간 도로 혼잡도를 기반으로 한 자동 추천 기능은 제거하고, 상대적으로 안정적인 공공데이터와 생활 인프라 데이터를 중심으로 도시를 분석하도록 변경했습니다.

이를 통해 분석 결과의 일관성과 신뢰성을 높이고, 장기적인 도시계획에 적합한 서비스를 구현했습니다.
