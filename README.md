...
> 최종 결과물은 각자 만든 코드의 모음이 아니라, 이 계약대로 연결되어 실행되는 하나의 서비스다.
        ...
        React-->>User: 입력 오류와 수정 안내 표시
    else 입력값이 정상임
        Boot->>Boot: Java로 도시 점수 계산
        Boot->>AI: 점수 설명과 계획안 3개 요청

        alt AI 호출 또는 구조화 출력 실패
            AI-->>Boot: 호출 실패
            Boot->>Boot: DB 저장 취소·트랜잭션 롤백
            Boot-->>React: 500 AI_ANALYSIS_FAILED
            React-->>User: 실패 안내와 다시 시도 버튼 표시
        else AI 분석 성공
            AI-->>Boot: UrbanAiResult 구조화 응답
            Boot->>DB: 입력·점수·AI 결과 저장
            DB-->>Boot: 분석 ID 반환
            Boot-->>React: 200 분석 결과 JSON
            React->>React: success 상태로 변경
            React-->>User: 점수·인사이트·계획안 표시
        end
    end
```
...
1. React가 `UrbanAnalysisRequest`를 백엔드로 전송합니다.
2. Spring Boot가 입력을 검사합니다.
3. Java가 도시 점수를 `0~100` 범위로 계산합니다.
4. Gemini는 계산된 숫자를 바꾸지 않고 설명과 세 가지 계획안을 작성합니다.
5. 성공한 결과만 DB에 저장합니다.
6. React가 응답을 받아 대시보드에 표시합니다.
...
- 도시 종합·교통·환경·경제·생활 점수
- AI 분석 요약과 분야별 인사이트
- `BALANCED`, `ECO`, `BUDGET` 계획안 3개
- 저장된 분석 ID와 생성 시각
...
| 상태 코드 | 오류 코드 | 의미 |
|---|---|---|
| `400` | `INVALID_INPUT` | 필수값 누락 또는 값의 범위 오류 |
| `404` | `ANALYSIS_NOT_FOUND` | 저장된 분석 결과를 찾을 수 없음 |
| `500` | `AI_ANALYSIS_FAILED` | Gemini 호출 또는 구조화 출력 실패 |
| `500` | `INTERNAL_ERROR` | DB 저장 또는 서버 내부 오류 |

자세한 요청·응답 필드와 DB 설계는 [`docs/design.md`](docs/design.md)를 참고합니다.
