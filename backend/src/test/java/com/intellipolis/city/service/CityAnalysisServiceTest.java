...
class CityAnalysisServiceTest {
 ...
 @Test void planWithoutVerifiedSitesDoesNotInventScoreGain(){var a=service.create(service.sample());assertThat(a.plans()).allSatisfy(p->{assertThat(p.facilities()).isEmpty();assertThat(p.expectedScores().overall()).isGreaterThanOrEqualTo(a.currentScores().overall());});}
 ...
 @Test void planUsesOnlyNeededFacilitiesAndMeasuredRoad(){var r=service.sample();var sites=List.of(new MapCoordinate(35.15,129.15),new MapCoordinate(35.16,129.16),new MapCoordinate(35.17,129.17));var line=List.of(List.of(129.15,35.15),List.of(129.17,35.17));var observations=List.of(new RoadObservation("1","중앙로",12d,1200d,"중앙교차로",45d,80L,line));var input=new CityAnalysisRequest(r.cityName(),r.districtName(),r.population(),r.areaKm2(),r.elderlyRatio(),r.youthRatio(),r.parkAreaRatio(),r.hospitalCount(),r.schoolCount(),r.transitHubCount(),r.averageHospitalDistanceKm(),r.averageParkDistanceKm(),r.averageTransitDistanceKm(),r.congestedRoads(),r.totalBudget(),r.priorityGoals(),r.mapCenter(),List.of(),sites,observations);var plan=service.create(input).plans().stream().filter(p->p.planType()==PlanType.BALANCED).findFirst().orElseThrow();assertThat(plan.facilities()).hasSize(3);assertThat(plan.roads()).hasSize(1);assertThat(plan.roads().getFirst().coordinates()).isEqualTo(line);assertThat(plan.roads().getFirst().improvementType()).isEqualTo(RoadImprovementType.SIGNAL_OPTIMIZATION);assertThat(plan.roads().getFirst().landImpact()).contains("철거를 전제로 하지 않음");}
}
